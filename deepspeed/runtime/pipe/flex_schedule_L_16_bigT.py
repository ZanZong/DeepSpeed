...
class TrainFlexSchedule(PipeSchedule):
    ...
    def interrupt_info(self):
        ...
        stage_breaks = {1: [25, 51]} # {stage_id:[break_layer0, break_layer1, ...]}
        return stage_breaks
    ...
    def static_stage0_backward(self, buffer_id):
        cmds = []
        cmds.append(RecvGrad(None, kwargs={'buffer_id': buffer_id, 'send': split_succ, 'recv': split}))
        cmds.append(SendActivation(None, kwargs={'buffer_id': buffer_id % 2, 'send': split, 'recv': split_succ}))
        cmds.append(BackwardPass(None, kwargs={'buffer_id': buffer_id, 'left': 0, 'right': split}))
        return cmds
        
    def static_stage1_forward(self, buffer_id):
        cmds = []
        cmds.append(ForwardPass(None, kwargs={'buffer_id': buffer_id, 'start': 26, 'end': 51}))
        cmds.append(RecvActivation(None, kwargs={'buffer_id': buffer_id, 'send': split, 'recv': split_succ}))
        cmds.append(ForwardPass(None, kwargs={'buffer_id': buffer_id, 'start': split_succ, 'end': 25}))
        cmds.append(ForwardPass(None, kwargs={'buffer_id': buffer_id, 'start': 52, 'end': 52}))
        return cmds
        
    def static_stage1_backward(self, buffer_id):
        cmds = []
        cmds.append(BackwardPass(None, kwargs={'buffer_id': buffer_id, 'left': 52, 'right': 52}))
        cmds.append(BackwardPass(None, kwargs={'buffer_id': buffer_id, 'left': split_succ, 'right': 25}))
        cmds.append(SendGrad(None, kwargs={'buffer_id': buffer_id, 'send': split_succ, 'recv': split}))
        cmds.append(BackwardPass(None, kwargs={'buffer_id': buffer_id, 'left': 26, 'right': 51}))
        return cmds
    ...
    def steps(self):
        ...
        for step_id in range(total_steps):
            ...
            if self.stage_id == 0:
                ...
                cmds.append(LoadMicroBatch(0))
                cmds.append(ForwardPass(None, kwargs={'buffer_id': 0, 'start': 26, 'end': 51}))
                cmds.append(RecvActivation(None, kwargs={'buffer_id': 0, 'send': split, 'recv': split_succ}))
                cmds.append(ForwardPass(None, kwargs={'buffer_id': 0, 'start': split_succ, 'end': 25}))
                cmds.append(ForwardPass(None, kwargs={'buffer_id': 0, 'start': 52, 'end': 52}))
                cmds.append(BackwardPass(None, kwargs={'buffer_id': 0, 'left': 52, 'right': 52}))
                cmds.append(BackwardPass(None, kwargs={'buffer_id': 0, 'left': split_succ, 'right': 25}))
                ...
                # to be planed
                cmds.append(LoadMicroBatch(1))
                cmds.append(ForwardPass(None, kwargs={'buffer_id': 1, 'start': 26, 'end': 51}))
                cmds.append(ForwardPass(None, kwargs={'buffer_id': 1, 'start': split_succ, 'end': 25}))
                cmds.append(ForwardPass(None, kwargs={'buffer_id': 1, 'start': 52, 'end': 52}))
                cmds.append(BackwardPass(None, kwargs={'buffer_id': 1, 'left': 52, 'right': 52}))
                cmds.append(BackwardPass(None, kwargs={'buffer_id': 1, 'left': split_succ, 'right': 25}))
                cmds.append(SendGrad(None, kwargs={'buffer_id': 1, 'send': split_succ, 'recv': split}))
                cmds.append(BackwardPass(None, kwargs={'buffer_id': 0, 'left': 26, 'right': 51}))
                cmds.append(BackwardPass(None, kwargs={'buffer_id': 1, 'left': 26, 'right': 51}))
